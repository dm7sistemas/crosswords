...
+ (PopoverView *)showPopoverAtPoint:(CGPoint)point inView:(UIView *)view withAttributedText:(NSAttributedString *)text delegate:(id<PopoverViewDelegate>)delegate {
    PopoverView *popoverView = [[PopoverView alloc] initWithFrame:CGRectZero];
    [popoverView showAtPoint:point inView:view withAttributedText:text];
    popoverView.delegate = delegate;
    [popoverView RELEASE];
    return popoverView;
}
...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view withAttributedText:(NSAttributedString *)text
{
    UIFont *font = kTextFont;
    
    NSStringDrawingContext* context = [[NSStringDrawingContext alloc] init];
    CGSize screenSize = [self screenSize];
    CGSize textSize = [text boundingRectWithSize:CGSizeMake(screenSize.width - kHorizontalMargin*4.f, 1000.f)
