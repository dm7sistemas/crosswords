...
- (void)setSelectedClue:(PuzzleClue *)selectedClue {
    if (![self.selectedClue isEqual:selectedClue]) {
        ...
        if (selectedClue) {
            ...
            CGRect a2 = CGRectMake(cellsLeft + col * cellSize, cellsTop + row * cellSize, cellSize, cellSize);
            CGRect clueArea = CGRectUnion(a1, a2);
            NSString* letterCounts = selectedClue.answerLetterCounts;
            NSString* seeAlsos = selectedClue.answerSeeAlsos;
            ...
            NSAttributedString* ats2 = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"\n%d %@, %@",
                                                                                   ...
                                                                                   seeAlsos ? [NSString stringWithFormat:@"%@ (%@)", letterCounts, seeAlsos] : letterCounts]
            ...
            for (PuzzleClue* siblingClue in selectedClue.siblingClues) {
                NSUInteger row = siblingClue.row;
                NSUInteger col = siblingClue.column;
                CGRect a1 = CGRectMake(cellsLeft + col * cellSize, cellsTop + row * cellSize, cellSize, cellSize);
                
                if (siblingClue.across)
                    col += siblingClue.length - 1;
                else
                    row += siblingClue.length - 1;
                CGRect a2 = CGRectMake(cellsLeft + col * cellSize, cellsTop + row * cellSize, cellSize, cellSize);
                CGRect clueArea = CGRectUnion(a1, a2);
                NSString* letterCounts = siblingClue.answerLetterCounts;
                NSString* seeAlsos = siblingClue.answerSeeAlsos;
                
                NSMutableAttributedString* ats1 = [[NSMutableAttributedString alloc] initWithString:siblingClue.displayClue
                                                                                         attributes:@{NSFontAttributeName: [UIFont boldSystemFontOfSize:18.0],
                                                                                                      NSForegroundColorAttributeName: [UIColor blackColor]}];
                NSAttributedString* ats2 = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"\n%d %@, %@",
                                                                                       (int)siblingClue.gridNumber,
                                                                                       siblingClue.across ? @"across" : @"down",
                                                                                       seeAlsos ? [NSString stringWithFormat:@"%@ (%@)", letterCounts, seeAlsos] : letterCounts]
                                                                           attributes:@{NSFontAttributeName: [UIFont italicSystemFontOfSize:12.0],
                                                                                        NSForegroundColorAttributeName: [UIColor grayColor]}];
...
#warning TODO
    ...
    CGFloat cellsLeft = (width - cellsWidth) / 2.0;
    CGFloat cellsTop = (height - cellsHeight) / 2.0;
    CGRect selectedArea = self.selectedClue ? self.selectedClue.area : CGRectMake(-1.0, -1.0, 0.0, 0.0);
    NSArray* siblingClues = self.selectedClue.siblingClues;
    NSArray* interectingClues = self.selectedClue.intersectingClues;
    ...
    for (NSInteger i = 0; i < rows * cols; ++i) {
        ...
        if ([letter isEqualToString:@"."]) {
            ...
            if (CGRectIntersectsRect(selectedArea, p)) {
                ...
                for (PuzzleClue* siblingClue in siblingClues) {
                    CGRect siblingArea = siblingClue.area;
                    if (CGRectIntersectsRect(siblingArea, p)) {
                        NSRect r = NSInsetRect(NSMakeRect(cellsLeft + col * cellSize, cellsTop + cellsHeight - (row * cellSize) - cellSize, cellSize, cellSize), .5, .5);
                        [context setFillColor:siblingCellColor];
                        [context fillRect:r];
                    }
                }
    ...
    for (PuzzleClue* clue in self.puzzle.cluesAcross.allValues) {
        ...
        for (NSUInteger col = CGRectGetMinX(area); col < CGRectGetMaxX(area); ++col) {
            ...
            if (area.origin.y > 0 &&
                ...
                ![self _row:area.origin.y - 1 column:col inRectanges:intersectingRects]) {
    ...
    for (PuzzleClue* clue in self.puzzle.cluesDown.allValues) {
        CGRect area = clue.area;
        NSArray* words = clue.words;
        NSArray* interectingCluesForClue = clue.intersectingClues;
        NSArray* intersectingRects = [interectingCluesForClue valueForKey:@"area"];
        ...
        for (NSUInteger row = CGRectGetMinY(area); row < CGRectGetMaxY(area); ++row) {
            ...
            if (area.origin.x > 0 &&
                ...
                ![self _row:row column:area.origin.x - 1 inRectanges:intersectingRects]) {
